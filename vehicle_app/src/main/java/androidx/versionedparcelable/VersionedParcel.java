...
public abstract class VersionedParcel {
    ...
    public void writeException(Exception e2, int fieldId) {
        setOutputField(fieldId);
        if (e2 == null) {
            writeNoException();
            return;
        ...
        int code = 0;
        if ((e2 instanceof Parcelable) && e2.getClass().getClassLoader() == Parcelable.class.getClassLoader()) {
            code = EX_PARCELABLE;
        } else if (e2 instanceof SecurityException) {
            code = -1;
        } else if (e2 instanceof BadParcelableException) {
            code = -2;
        ...
        } else if (e2 instanceof NetworkOnMainThreadException) {
        ...
        writeInt(code);
        if (code != 0) {
            writeString(e2.getMessage());
            if (code == EX_PARCELABLE) {
                writeParcelable((Parcelable) e2);
            }
